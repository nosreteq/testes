Sim, várias. Organizando por camada de causa — cada uma cobre uma hipótese diferente das que já discutimos:
...
Se NLS_COMP/NLS_SORT/DEFAULT_COLLATION divergirem aqui, é a causa mais provável — já discutido, mas essa é a fonte oficial de onde vem o default (spfile/logon trigger), não só o que a sessão herdou.
...
Ordem recomendada de investigação, do mais provável ao mais raro no seu cenário:

nls_database_parameters comparado 19c×23ai (NLS_COMP/SORT/COLLATION)
v$system_fix_control filtrando SET/UNION/SORT
Optimizer parameters gerais
Plan Baselines/SQL Patches pré-existentes
Parâmetros _underscore ocultos
